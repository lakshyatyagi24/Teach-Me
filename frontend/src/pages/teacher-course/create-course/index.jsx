...
const createCourse = ({ isUpdate = false }) => {
      ...
      if (res) {
        if (data?.grade <= 85) {
          ...
          try {
            dispatch(setGlobalLoader(true));
            axios({
              url: `${process.env.REACT_APP_BECKEND_URL}/api/teacher-Course`,
              method: "POST",
              headers: {
                authorization: `Bearer ${token}`,
              },
  ...
  const handleUpdate = () => {
    form.validateFields().then(async (res) => {
      if (res) {
        dispatch(setGlobalLoader(true));
        axios({
          url: `${process.env.REACT_APP_BECKEND_URL}/api/teacher-Course/update`,
          method: "POST",
          headers: {
            authorization: `Bearer ${token}`,
          },
  ...
  const handleSwitch = (value) => {
    ...
    dispatch(setGlobalLoader(true));
    axios({
      url: `${process.env.REACT_APP_BECKEND_URL}/api/teacher-Course/status`,
      method: "POST",
      headers: {
        authorization: `Bearer ${token}`,
      },
